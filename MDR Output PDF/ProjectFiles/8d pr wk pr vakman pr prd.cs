...
namespace MDR2PDF
{
    public partial class _8d_pr_wk_pr_vakman_pr_prd : UserControl
    {
        ...
        public _8d_pr_wk_pr_vakman_pr_prd()
        {
            InitializeComponent();
            // Verander het tabelkopje even, hier kan beter Vakmannen staan ipv Projecten
            htmlTabel = htmlTabel.Replace("Projecten", "Vakmannen");
        }
            ...
            do // Start met totaal-periode; Neem daarna alle weekperiodes
            {
                ...
                var _VakmanSelecties = selxMANDAGEN
                    ...
                    .Select(x => new { VID = x.VakmanId
                        ...
                        , Vakman = Vakmannen.Where( z => z.VakmanId == x.VakmanId ).FirstOrDefault()
                    }).Distinct().ToList();
